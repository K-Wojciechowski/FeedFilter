...
import type { Decision, Feed, FeedFilteringResult, FeedUpdate, Rule } from "./models.ts";
...
export default function EditPage(props: EditPageProps): ReactElement {
  ...
  const [feedId, setFeedId] = useState<string>(props.editedFeed?.feedId ?? "");
  const [description, setDescription] = useState<string>(props.editedFeed?.description ?? "");
  const [url, setUrl] = useState<string>(props.editedFeed?.url ?? "");
  const [defaultDecision, setDefaultDecision] = useState<Decision>(props.editedFeed?.defaultDecision ?? "accept");
  const [rules, setRules] = useState<EditableRule[]>(props.editedFeed != undefined ? [...props.editedFeed.rules.map((x, i) => ({
    ...x,
    index: i + 1,
    id: i,
    deleted: false,
    new: false
  })), newEmptyRule(props.editedFeed.rules.length)] : [newEmptyRule(0)]);
  ...
  const moveUp = useCallback(
    (ruleId: number) => setRules((rr: EditableRule[]): EditableRule[] => {
  ...
  const updateRule = useCallback(
    (ruleId: number, update: any) => setRules(rr => {
      const updatedRules: EditableRule[] = [...rr.map(r => r.id == ruleId ? { ...r, ...update, new: false } : r)];
  ...
  const buildFeedUpdate = useCallback((cleanUpIndexes: boolean) => {
    const newRules = rules
      .filter(r => !r.deleted && !r.new)
      .map((r, i) => {
        const { id: _1, deleted: _2, ...rest } = r;
  ...
  return <>
    <PageHeader title={props.mode === "create" ? "Create Feed" : "Edit Feed"} showBackButton={true} />
        ...
        <Paper sx={{ padding: 2, height: "100%", display: "flex" }}>
          <Grid container spacing={2}>
            ...
            <FormControl fullWidth>
              ...
              <DecisionPicker value={defaultDecision} setValue={setDefaultDecision} id="defaultDecision" />
            </FormControl>
          </Grid>
        </Paper>
      </Grid>
      <Grid size={6}>
        <Paper sx={{ height: "100%", display: "flex" }}>
          <Box>
            <Box sx={{ padding: 2 }}>
              <Box>
                <Button variant="outlined" loading={testState.loading} onClick={() => runTest()} sx={{ mr: 1 }}>Test
                  Feed</Button>
                <Button variant="text" disabled={testState.loading || testState.result == undefined} onClick={() => setTestState(s => ({...s, xmlOpen: true}))}>Show XML</Button>
                {testState.result != undefined && <ShowXmlDialog feedId={feedId} originalXml={testState.result!.originalXml} filteredXml={testState.result!.filteredXml} open={testState.xmlOpen} setOpen={(xmlOpen) => setTestState(s => ({...s, xmlOpen}))} />}
              </Box>
              {testState.result == undefined && testState.error == undefined &&
                <Box>To test the rules configured below, press <strong>Test Feed</strong>. The titles of posts will be
                  displayed alongside their decisions and IDs of rules that caused the decision.</Box>}
              {testState.error != undefined && <Alert severity="error">{testState.error}</Alert>}
            </Box>
            {testState.result != undefined && <TableContainer component={Box} maxHeight={320}>
              <Table sx={{ maxHeight: 400 }} size="small" aria-label="test table">
                ...
                <TableBody>
                  {testState.result.entryResults.map((result, i) => (
                    ...
                    >
                      <TableCell component="th" scope="row">{result.entryTitle}</TableCell>
                      <TableCell><Typography
                        sx={{ fontSize: "inherit" }}
                        color={decisionColor(result.decision)}>{decisionName(result.decision)}</Typography></TableCell>
          ...
          {rules.map(rule => (
            <TableRow
              ...
              sx={{
                ...
                background: rule.deleted ? "rgba(255, 0, 0, 0.1)" : rule.new ? "rgba(0, 128, 0, 0.1)" : undefined
              }}
            >
              <TableCell component="th" scope="row">{rule.new ? "+" : rule.index}</TableCell>
              <TableCell>
                <Select
                  fullWidth
                  value={rule.field}
                ...
                {rule.field === "custom" && <>
                  <TextField
                    ...
                    onChange={e => updateRule(rule.id, { customXPath: e.target.value === "" ? undefined : e.target.value })}
                    size="small"
                    sx={{ mt: 1 }}
                    disabled={rule.deleted}
                  />
                  <TextField
                    ...
                    onChange={e => updateRule(rule.id, { testedAttributeName: e.target.value === "" ? undefined : e.target.value })}
                    size="small"
                    sx={{ mt: 1 }}
                    disabled={rule.deleted}
                  />
              ...
              <TableCell>
                <DecisionPicker value={rule.decision} setValue={d => updateRule(rule.id, { decision: d })}
                                disabled={rule.deleted}
                />
              </TableCell>
              <TableCell>
                <OutlinedInput
                  fullWidth
                  value={rule.comment ?? ""}
                  onChange={e => updateRule(rule.id, { comment: e.target.value === "" ? undefined : e.target.value })}
                  disabled={rule.deleted}
                />
              </TableCell>
              <TableCell sx={{ width: actionCellWidth }} align="right">
                <Tooltip title="Move Up"><span>
                  <IconButton aria-label="up" size="small" disabled={rule.deleted || rule.new || rule.index === 1}
                ...
                <Tooltip title={rule.deleted ? "Restore" : "Delete"}><span>
                <IconButton aria-label="delete" size="small"
                            onClick={() => updateRule(rule.id, { deleted: !rule.deleted })}
                            disabled={rule.new}>
                  {rule.deleted ? <RestoreFromTrashIcon fontSize="inherit" /> : <DeleteIcon fontSize="inherit" />}
                </IconButton>
                  </span></Tooltip>
              </TableCell>
            </TableRow>))}
    ...
    <Grid container sx={{ mt: 2, mb: 2 }}>
      <Grid display="flex" justifyContent="center" alignItems="center" size="grow">
        <Button variant="contained" startIcon={<SaveIcon />} size="large" onClick={() => doSave()}>
          Save
        </Button>
      </Grid>
    </Grid>
