...
type Page = "auth" | "list" | "create" | "edit";

interface FeedFilterState {
  page: Page;
  feeds: Feed[];
  loading: boolean;
  editedFeed: Feed | undefined;
  token: string | undefined;
  goToPage: (page: Page) => void;
  startEditing: (feed: Feed) => void;
  init: (token: string) => Promise<void>;
  save: (feedId: string, feedUpdate: FeedUpdate) => Promise<void>;
  delete: (feedId: string) => Promise<void>;
}

const sortFeeds = (feeds: Feed[]): Feed[] => feeds.sort((a, b) => {
  if (a < b) {
    return -1;
  }
  if (a > b) {
...
const useFeedFilterStore = create<FeedFilterState>()(
  devtools(
    (set, get) => ({
      ...
      startEditing: (feed: Feed) => set({ page: "edit", editedFeed: feed }),
      ...
      delete: async (feedId: string) => {
        ...
        try {
          ...
          const currentFeeds = get().feeds;
          set({ feeds: currentFeeds.filter(feed => feed.feedId !== feedId), page: "list" });
        } finally {
          set({ loading: false });
        }
      }
